  ...
  try {
    ...
    for (const ticker of tickers) {
      ...
      try {
        ...
        const alphaResp = await axios.get(alphaUrl);
        ...
        stockData.alphaVantage = {
          ...
          changePercent: prevClose !== null ? ((price! - prevClose) / prevClose) * 100 : null
        };
      } catch {
        stockData.alphaVantage = { price: null, previousClose: null, changePercent: null, error: "Falha no Alpha Vantage" };
      }

      // --- Finnhub ---
      try {
        const finnhubUrl = `https://finnhub.io/api/v1/quote?symbol=${ticker}&token=${FINNHUB_KEY}`;
        const finnhubResp = await axios.get(finnhubUrl);
        const { c: current, pc: prevClose } = finnhubResp.data;

        stockData.finnhub = {
          price: current ?? null,
          previousClose: prevClose ?? null,
          changePercent: prevClose !== null ? ((current - prevClose) / prevClose) * 100 : null
        };
      } catch {
        stockData.finnhub = { price: null, previousClose: null, changePercent: null, error: "Falha no Finnhub" };
      }

      // --- Twelve Data ---
      try {
        const twelveUrl = `https://api.twelvedata.com/price?symbol=${ticker}&apikey=${TWELVE_DATA_KEY}`;
        const twelveResp = await axios.get(twelveUrl);
        ...
        stockData.twelveData = { price: null, previousClose: null, changePercent: null, error: "Falha no Twelve Data" };
      }
